...
//chức năng giỏ hàng
let listProductsHTML=document.querySelector(".right-content-block");
let listProducts = [];
let carts = [];
let ListCartHTML = document.querySelector('.cart-container');
let iconCartSpan = document.querySelector('.cart-icon1 span');
    ...
    if (listProducts.length >0){
        listProducts.forEach(product => {
            ...
            <div class="product-image">
                    <span class="product-sale"><p>${product.sale}</p></span>   
                    <img src="${product.image}" alt="">
                    <a class="btn-add-new-item" onclick="showSuccessToast();">Thêm vào giỏ hàng</a>
                    </div>
                    <div class="product-name">
                        <a href="./product-detail.html" class="product-name">${product.name}</a>
                    </div>
                    <div class="product-info">
                        <ul class="product-info-list">
                        <li class="age"><p>${product.age}+</p></li>
                        <li class="player"><p>${product.player}</p><img src="./Assets/Icons/users.svg" alt=""></li>
                        <li class="time"><p>${product.time}</p><img src="./Assets/Icons/clock.svg" alt=""></li>
                    </ul>
                    </div>
                    ...
                    <div class="btn-mobile-add" onclick="showSuccessToast()">Thêm vào giỏ</div>
            `;
...
listProductsHTML.addEventListener('click', (event) => {
    ...
    if(positionClick.classList.contains('btn-add-new-item')){
...
const addCartToHTML = () => {
    ...
    if (carts.length > 0){
        carts.forEach(cart => {
            ...
            newCart.classList.add('card-item');
            ...
            <div class="card-line">
            <div class="card-quantity">x${cart.quantity}</div>
            <div class="card-price">${info.price * cart.quantity}.000 đ</div>
            </div>
            </div>
            `;
    ...
    totalCartPrice.innerHTML = totalPrice + '.000 đ';
...
function showSuccessToast() {
  ...
  const btnOpenNav1 = document.getElementById('mobile')
  const btnCloseNav1 = document.querySelector('.nav-close1')
  const sideNav1 = document.querySelector('.side-filter')
  const backdrop2 = document.querySelector('.backdrop2')
  btnOpenNav1.addEventListener('click',openNav)
  btnCloseNav1.addEventListener('click',closeNav)
  
  function openNav(){
    sideNav1.classList.add('open')
    backdrop2.classList.add('show')
  }
  
  function closeNav(){
    sideNav1.classList.remove('open')
